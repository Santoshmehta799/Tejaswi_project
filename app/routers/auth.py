...
def login(user_credentials: LoginSchema, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(data={
        "sub": str(user.id),
        "role": user.role  # ✅ Convert enum to string
    })
...
def create_sticker(
    ...
    try:
        ...
        if hasattr(data, 'serial_number') and data.serial_number:
            ...
            serial_number = get_next_serial_number_from_model(db, data.shift, data.production_date)

        # Generate product number first
        product_number = generate_product_number(data.shift, data.production_date, serial_number)
...
class AdminConfigService:
    ...
    def create_colour_item(self, config_type: str, name: str, db: Session, is_white: Optional[bool] = None):
    ...
    def update_colour_item(self, config_type: str, item_id: int, is_white: bool, db: Session):
...
def manage_config(
    ...
    try:
        if request.action == "create":
            ...
            if not request.id:
                raise HTTPException(status_code=400, detail="ID is required for update action")
            if request.is_white is None:
                raise HTTPException(status_code=400, detail="is_white field is required for update action")
...
def get_next_serial_number_from_model(db: Session, shift: str, production_date: date) -> str:
    ...
    existing_serials = db.query(StickerGenerator.serial_number).filter(
...
def generate_product_number(shift: str, production_date: date, serial_number: str) -> str:
...
def get_all_inventory_records(db: Session = Depends(get_db),current_user = Depends(get_current_user)):
    """
...
def get_sticker_by_product_number(
    ...
    try:
        ...
        result = db.query(
            ...
            Quality, StickerGenerator.quality_id == Quality.id
        ).join(
            Colour, StickerGenerator.colour_id == Colour.id
        ).filter(
            StickerGenerator.product_number == product_number
        ).first()
        ...
        existing = db.query(ScannedProduct).filter_by(
        ...
        scanned_products = db.query(ScannedProduct).filter_by(
            user_id=current_user.id
        ).order_by(ScannedProduct.created_at.asc()).all()
...
def get_all_scanned_products(
    ...
    try:
        ...
        scanned_products = db.query(ScannedProduct).filter_by(
            user_id=current_user.id
        ).order_by(ScannedProduct.created_at.desc()).all()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving scanned products: {str(e)}"
        )
...
def delete_scanned_product(
    ...
    product = db.query(ScannedProduct).filter(
        ScannedProduct.product_number == product_number
    ).first()
...
def delete_sticker_by_product_number(
    ...
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)):
    sticker = db.query(StickerGenerator).filter(StickerGenerator.product_number == product_number).first()
    ...
    return {"detail": f"Product with product_number '{product_number}' has been deleted successfully"}
